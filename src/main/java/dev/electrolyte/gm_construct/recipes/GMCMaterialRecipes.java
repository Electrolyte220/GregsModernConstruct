    ...
    public void register(Consumer<FinishedRecipe> provider) {
        for(Material material : GTMaterialHelper.getRegisteredMaterials()) {
            ...
            if(GMCConfig.GENERATE_MELTING_CASTING_RECIPES.get()) {
                MaterialMeltingRecipeBuilder.material(GMConstruct.materialId(material.getName()), material.getFluid().getFluidType().getTemperature() - 300, new FluidStack(material.getFluid(), 144))//todo: check temp
